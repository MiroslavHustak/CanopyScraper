...
module MyCanopyChrome =
    ...
    let internal canopyResult () =
        IO (fun ()
                ...
                let startHeadlessChrome () =
                    try
                        let service =
                            match Environment.OSVersion.Platform = PlatformID.Win32NT with
                            | true  -> ChromeDriverService.CreateDefaultService pathToDriver
                            | false -> ChromeDriverService.CreateDefaultService()  //The path is in the environment variable in the container, so default service will find it
                        ...
                        let driver = new ChromeDriver(service, options)
                    ...
                    | ex                         
                        ->
                        eprintfn "CRITICAL: Failed to start Chrome driver: %s" ex.Message
